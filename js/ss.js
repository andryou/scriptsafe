...
function blockreferrer() {
	$("a[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (thirdParty(elSrc)) { $(this).attr("rel","noreferrer"); } $(this).attr("data-scriptsafe",timestamp); });
}
...
function ScriptSafe() {
	...
	if (SETTINGS['APPLET'] == 'true') $("applet[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = $(this).attr('code'); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.extension.sendRequest({reqtype: "update-blocked", src: elSrc, node: 'APPLET'}); $(this).remove(); } else { chrome.extension.sendRequest({reqtype: "update-allowed", src: elSrc, node: 'APPLET'}); $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['VIDEO'] == 'true') $("video[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.extension.sendRequest({reqtype: "update-blocked", src: elSrc, node: 'VIDEO'}); removeMedia($(this)); } else { chrome.extension.sendRequest({reqtype: "update-allowed", src: elSrc, node: 'VIDEO'}); $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['AUDIO'] == 'true') $("audio[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc); if (postLoadCheck(elSrc.toLowerCase())) { chrome.extension.sendRequest({reqtype: "update-blocked", src: elSrc, node: 'AUDIO'}); removeMedia($(this)); } else { chrome.extension.sendRequest({reqtype: "update-allowed", src: elSrc, node: 'AUDIO'}); $(this).attr("data-scriptsafe",timestamp); } } });
	/* handled by background page, but clean up elements */
	if (SETTINGS['IFRAME'] == 'true') $("iframe[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { $(this).remove(); } else { $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['OBJECT'] == 'true') $("object[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { $(this).remove(); } else { $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['EMBED'] == 'true') $("embed[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { $(this).remove(); } else { $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['IMAGE'] == 'true') $("picture[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { $(this).remove(); } else { $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['IMAGE'] == 'true') $("img[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { $(this).remove(); } else { $(this).attr("data-scriptsafe",timestamp); } } });
	if (SETTINGS['SCRIPT'] == 'true' && SETTINGS['EXPERIMENTAL'] == '0') {
		clearUnloads();
		$("script[data-scriptsafe!='"+timestamp+"']").each(function() { var elSrc = getElSrc(this); if (elSrc) { elSrc = relativeToAbsoluteUrl(elSrc.toLowerCase()); if (postLoadCheck(elSrc)) { chrome.extension.sendRequest({reqtype: "update-blocked", src: elSrc, node: 'SCRIPT'}); $(this).remove(); } else { if (elSrc.substr(0,11) != 'javascript:' && elSrc.substr(0,17) != 'chrome-extension:') { chrome.extension.sendRequest({reqtype: "update-allowed", src: elSrc, node: "SCRIPT"}); $(this).attr("data-scriptsafe",timestamp); } } } });
